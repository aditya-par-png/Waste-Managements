...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
      ...
      $stmt = $conn->prepare("INSERT INTO users (first_name, last_name, email, password, otp, otp_expiry) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssss", $firstName, $lastName, $email, $hashedPassword, $otp, $otpExpiry);
            ...
            if ($stmt->execute()) {
                ...
                try {
                    ...
                    $mail->SMTPAuth = true;
                    $mail->Username = 'carryislive123@gmail.com'; // ✅ your SMTP email
                    $mail->Password = 'vkpy lkvd adjn vkzg';    // ✅ your SMTP password
                    $mail->SMTPSecure = 'tls';
                    $mail->Port = 587;
                    ...
                    $mail->Body = "Hello <b>$firstName</b>,<br><br>Your OTP for signup is: <strong>$otp</strong><br><br>This OTP will expire in 10 minutes.";
...
<body>
  ...
  <!-- Slogan -->
  <div class="slogan">Where Waste is Managed, and the Environment is Cleaned!</div>
